...
public class ProjectileThrower : MonoBehaviour {
    ...
    {
        bool rst = false;
        //var target = FindObjectsOfType<Rigidbody2D>();
    ...
    /// <summary>
    /// Reloads the level if the projectile has been fired
    /// or the player presses the escape key
    /// </summary>
    internal void Update()
    {
        if (firingState == FiringState.Firing&& (!IsActive(myRigidBody))&& (!WaitingForPhysicsToSettle()))
        {
            ResetForFiring();
        }
